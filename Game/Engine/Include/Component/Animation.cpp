...
bool CAnimation::CreateBoneTexture()
{
	...
	tDesc.ArraySize = 1;
	tDesc.BindFlags = D3D12_BIND_SHADER_RESOURCE;
...
ID3D12ShaderResourceView * CAnimation::GetBoneTexture() const
{
	return m_pBoneRV;
}
...
int CAnimation::Update(float fTime)
{
	...
	if (!m_bEnd)
	{
		...
		CMLIST->Map(m_pBoneTex, 0, D3D12_MAP_WRITE_DISCARD, 0, &tMap);

		memcpy(tMap.pData, &vecBones[0], sizeof(Matrix) * vecBones.size());

		CMLIST->Unmap(m_pBoneTex, 0);
	}
