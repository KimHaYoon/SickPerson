...
void CMaterial::AddMultiTexture(const string & strSmpKey, ID3D12ShaderResourceView * pSRV, int iRegTex, int iRegSmp)
{
...
{
	ID3D12ShaderResourceView*	pSRV = NULL;
	ID3D12SamplerState*			pSmp = NULL;
	...
	if (m_pDifInfo)
	{
		if (m_pDifInfo->pTexture)
		{
			if (iShaderConstantType & SCT_VERTEX)
				CMLIST->VSSetShaderResources(m_pDifInfo->iRegisterTex, 1, &pSRV);

			if (iShaderConstantType & SCT_PIXEL)
				CMLIST->PSSetShaderResources(m_pDifInfo->iRegisterTex, 1, &pSRV);
		}
	}
	...
	if (m_pNormalInfo)
	{
		...
		{
			if (iShaderConstantType & SCT_PIXEL)
				CMLIST->PSSetShaderResources(m_pNormalInfo->iRegisterTex, 1, &pSRV);
		}
	}
	...
	if (m_pSpecularInfo)
	{
		...
		{
			if (iShaderConstantType & SCT_PIXEL)
				CMLIST->PSSetShaderResources(m_pSpecularInfo->iRegisterTex, 1, &pSRV);
		}
	}
	...
	for (size_t i = 0; i < m_vecMultiTex.size(); ++i)
	{
		...
		{
			if (iShaderConstantType & SCT_PIXEL)
				CMLIST->PSSetShaderResources(m_vecMultiTex[i]->iRegisterTex, 1, &pSRV);
		}
	}
}
