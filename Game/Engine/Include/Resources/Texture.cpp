	...
	const vector<string>& vecFullPath)
{
	...
	vector<ID3D12Texture2D*>	vecTex;

	for (size_t i = 0; i < m_vecImage.size(); ++i)
	{
		ID3D12Texture2D*	pTex = NULL;

		if (FAILED(CreateTextureEx(DEVICE, m_vecImage[i]->GetImages(),
			m_vecImage[i]->GetImageCount(), m_vecImage[i]->GetMetadata(),
			D3D12_USAGE_STAGING, 0,
			D3D12_CPU_ACCESS_WRITE | D3D12_CPU_ACCESS_READ, 0, FALSE, (ID3D12Resource**)&pTex)))
		{
	...
	D3D12_TEXTURE2D_DESC	tTexDesc = {};
	vecTex[0]->GetDesc(&tTexDesc);

	// 읽어온 이미지들로 TextureArray를 생성한다.
	D3D12_TEXTURE2D_DESC	tTexArr = {};
	...
	tTexArr.Usage = D3D12_USAGE_DEFAULT;
	tTexArr.BindFlags = D3D12_BIND_SHADER_RESOURCE;

	ID3D12Texture2D*	pTexArr = NULL;
	...
	for (size_t i = 0; i < vecTex.size(); ++i)
	{
			...
			++iMipLevel)
		{
			...
			CMLIST->Map(vecTex[i], iMipLevel, D3D12_MAP_READ,
				0, &tMap);

			CMLIST->UpdateSubresource(pTexArr,
				D3D12CalcSubresource(iMipLevel, i, tTexDesc.MipLevels),
				NULL, tMap.pData, tMap.RowPitch, tMap.DepthPitch);

			CMLIST->Unmap(vecTex[i], iMipLevel);
		}
	}

	D3D12_SHADER_RESOURCE_VIEW_DESC	tViewDesc = {};

	tViewDesc.Format = tTexArr.Format;
	tViewDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE2DARRAY;
...
void CTexture::SetTexture(int iRegister, int iShaderConstantType)
{
	if (iShaderConstantType & SCT_VERTEX)
		CMLIST->VSSetShaderResources(iRegister, 1, &m_pSRView);

	if (iShaderConstantType & SCT_PIXEL)
		CMLIST->PSSetShaderResources(iRegister, 1, &m_pSRView);
}
...
void CTexture::SetShaderResourceView(ID3D12ShaderResourceView * pSRV)
{
	m_pSRView->AddRef();
	m_pSRView = pSRV;
}
