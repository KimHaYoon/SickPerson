	...
	const char* pHeightMap, const string& strPathKey)
{
	...
	pMaterial->SetNormalTexInfo("Linear", strTexKey + "_N", 1, 1, pNormalName, strPathKey);
	pMaterial->SetSpecularTexInfo("Linear", strTexKey + "_S", 2, 2,	pSpecularName, strPathKey);
	...
	for (int i = 0; i < m_iNumZ; ++i)
	{
		for (int j = 0; j < m_iNumX; ++j)
		{
			...
			tVtx.vPos = Vector3((float)j, 
				(pPixel[i * m_iNumX + j] & 0x000000ff) * 0.03f, (float)(m_iNumZ - 1) - i);
...
int CLandScape::LateUpdate(float fTime)
{
	...
	if (pRenderer)
	{
		...
		pRenderer->UpdateCBuffer("LandScape", 12, sizeof(LANDSCAPECBUFFER), SCT_PIXEL, &tBuffer);
		SAFE_RELEASE(pRenderer);
	}
